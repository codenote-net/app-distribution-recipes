...
app.whenReady().then(async () => {
  const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "sqlcipher-windows-todo-test-"));
  ...
  try {
    ...
    database = undefined;
    assert.notEqual(fs.readFileSync(databasePath).subarray(0, 16).toString("utf8"), "SQLite format 3\u0000");
