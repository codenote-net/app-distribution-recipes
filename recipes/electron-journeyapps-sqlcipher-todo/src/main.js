...
app.setName("JourneyApps SQLCipher Todo");
...
function createWindow() {
  const window = new BrowserWindow({
    ...
    minWidth: 560,
    minHeight: 520,
    title: "JourneyApps SQLCipher Todo",
    webPreferences: {
...
app.whenReady().then(async () => {
  const databasePath = path.join(app.getPath("userData"), "todos.db");
  todoDatabase = await TodoDatabase.open(databasePath, DATABASE_KEY);
